...
from dataset.HistogramGenerator import generate_histograms, generate_hist_statistics
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx=None, crop_coords=None):
        ...
        # INPUT FEATURES
        stats = generate_hist_statistics(input_samples, device=self.device)
        input_tensor = stats['mean']                                # default input (3, H, W)

        if self.mode == "stat":
            # Concatenate variance and std to input channels
        ...
        # AOV
        if self.aov:
            input_tensor = torch.cat([input_tensor, albedo_tensor, normal_tensor], dim=0)       # stat: (9 + 6, H, W) img: (3 + 6, H, W)
...
class HistogramDataset(Dataset):
    ...
    def __getitem__(self, idx=None, crop_coords=None):
        ...
        hist, bin_edges = load_or_generate_histogram(
            ...
            hist_filename=hist_filename, 
            save_hist=self.hist_regeneration
        )
...
class CropHistogramDataset(Dataset):
    ...
    def __getitem__(self, idx=None, crop_coords=None):
        ...
        # less random than ImageDataset because we only select indices randomly per image !!!!
        indices = list(range(self.low_spp))
        random.shuffle(indices)
        input_idx = indices[:self.target_split]
        target_idx = indices[-self.target_split:]

        # Convert input samples to tensor: (target_split, 3, H, W)
        input_samples = spp1_img[input_idx]  # (N, 3, H, W)
        input_samples_tensor = torch.from_numpy(input_samples).float()
        ...
        input_samples_raw = input_cropped.permute(0, 2, 3, 1).numpy()  # (N, crop_h, crop_w, 3)
        ...
        bin_edges_tensor = torch.from_numpy(bin_edges).float()                  # (H, W, 3, bins)
        ...
        if self.supervised:
            ...
            target_samples = torch.from_numpy(spp1_img[target_idx]).float()  # (N, 3, H, W)
...
class HistogramBinomDataset(Dataset):
    def __init__(self, root_dir: str, crop_size: int = 128,
        ...
        for key, folder in scene_keys:
            ...
            hist_tensor = torch.from_numpy(hist).float()  # (H, W, 3, B)
            # 1. Collapse spatial dimensions and channels to get total bin counts
            bin_counts = hist_tensor.sum(dim=(0, 1, 2))  # shape: (B,)
...
class CropHistogramBinomDataset(Dataset):
    def __init__(self, root_dir: str, crop_size: int = 128,
        ...
        for key, folder in scene_keys:
            ...
            spp1_path = os.path.join(folder, spp1_file)
            spp1_samples = tifffile.imread(spp1_path)  # shape: (32, H, W, 3)
            self.spp1_samples[key] = spp1_samples
